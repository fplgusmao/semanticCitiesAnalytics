...
gulp.task('styles', ['clean-styles'], function () {
    ...
    return gulp
        ...
        .pipe($.inject(gulp.src(['*.less'], {read: false, cwd: config.styles}), {
...
gulp.task('wiredep', function () {
    ...
    return gulp
        ...
        .pipe(inject(config.otherDependenciesJs, 'components'))
 ...
 * optimize before handling image or fonts
 */
gulp.task('fast-build', ['fast-optimize', 'images', 'fonts'], function () {
    log('Building everything');

    var msg = {
        title: 'gulp fast build',
        subtitle: 'Deployed to the build folder',
        message: 'Running `gulp serve-build`'
    };
    del(config.temp);
...
/**
 * Optimize some files, skips minification of
 * lib and app, move to a build folder,
 * and inject them into the new index.html
 * @return {Stream}
 */
gulp.task('fast-optimize', ['inject', 'test'], function () {
    log('Optimizing the js, css, and html');
